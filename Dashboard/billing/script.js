     ...
     main.innerHTML = 'LOADING BILING INFO ......................'
      request.sendRequest('GET', '/client/billing   ')
        ...
        .catch(error => {console.error('Error loading billing history:', error)
            this.body.innerHTML = `AN UNEXPECTED ERROR OCCURED`;
...
const tableBody = document.querySelector('#billingHistoryTable');
    ...
    data.forEach(item => {
      ...
      row.innerHTML = `
        <td>${item.billing.date}</td>
        <td>${item.billing.service}</td>
        <td>$${item.billing.amount}</td>
        <td><button onclick="window.location.href='${item.receiptUrl}'">Download</button></td>
      `;
...
function updateCard(data)
{
    ...
    cardName.innerHTML = data.card.name
    cardImage.src = `/assets/images/${data.card.brand}.png`
    cardBrand.innerHTML = `${data.card.brand}`
    last4.innerHTML = `*************${data.card.last4}`
    expMonth.innerHTML = data.card.exp_month
    expYear.innerHTML = data.card.exp_year
